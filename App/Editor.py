...
class Savedata():
    ...
    def parse_savefile(jsondata):
        ...
        try:
            ...
            # TODO - add support for non 3-digit room numbers (not relevant for saves generated by normal gameplay)
        except:
            pass
        for f in ["eq00", "eq01"]:
...
class Editor():
    ...
    def export(self, slot):
        ...
        with open(join(self.path, self.get_savefile_name(slot)), "wb", buffering=0) as savefile:
            self.savedata.export_savefile(savefile, header)


    def get_savefile_name(self, slot):
        return f"hyperlight_recordofthedrifter_{slot}.sav" if platform.system() == "Linux" else f"HyperLight_RecordOfTheDrifter_{slot}.sav"
